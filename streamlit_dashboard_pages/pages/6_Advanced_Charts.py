...
from streamlit_dashboard_pages.shared import initialize_session_state, save_visualization, calculate_fairness_score, safe_set_page_config
...
if st.session_state.last_allocation_rows is not None and st.session_state.last_repos is not None:
    ...
    st.warning("👆 Run allocation first to generate Sankey diagram")
...
if st.session_state.last_allocation is not None:
    ...
    # Student × Topic Heatmap
    st.subheader("📊 Student × Topic Heatmap")
    ...
    if 'effective_cost' in df.columns and 'student' in df.columns and 'assigned_topic' in df.columns:
        ...
        # Find reasonable bounds for the color scale (excluding -10000)
        all_values = df['effective_cost'].values
        ...
        fig = go.Figure(data=go.Heatmap(
            ...
            zmin=zmin,
            zmax=zmax,
            hovertemplate='Student: %{y}<br>Topic: %{x}<br>Cost: %{z}<br>%{customdata}<extra></extra>',
            customdata=np.where(cost_pivot.values == -10000,
                               np.full(cost_pivot.shape, '🔧 Forced Assignment'),
                               np.full(cost_pivot.shape, 'Normal Assignment'))
        ))
        fig.update_layout(
            title=f"Effective Cost Heatmap - All {len(df)} Students (🔧 Forced topics shown at min of scale)",
    ...
    if 'assigned_coach' in df.columns and 'assigned_topic' in df.columns and 'effective_cost' in df.columns:
        ...
        # Filter out very negative values (likely from forced assignments)
    ...
    st.warning("👆 Run allocation first to generate heatmaps")
        ...
        if st.session_state.last_repos:
            ...
            for dept_id, dept in repo.departments.items():
                if dept_id in dept_data:
                    # Normalize legacy int format to dict format before updates
                    if isinstance(dept_data[dept_id], int):
                        used_val = dept_data[dept_id]
                        dept_data[dept_id] = {
                            'used': used_val,
                            'total': used_val,
                            'pct': 100
                        }
                    # Use desired_max as capacity, fallback to desired_min, or use assigned count
                    desired_max = getattr(dept, 'desired_max', None)
                    desired_min = getattr(dept, 'desired_min', None)
                    max_val = desired_max if isinstance(desired_max, (int, float)) and desired_max is not None else 0
                    min_val = desired_min if isinstance(desired_min, (int, float)) and desired_min is not None else 0
                    if max_val > 0:
                        capacity = int(max_val)
                    elif min_val > 0:
                        capacity = int(min_val)
                    else:
                        capacity = dept_data[dept_id]['used']
                    dept_data[dept_id]['total'] = capacity
        ...
        if gini_cost > 0.5:
            ...
            with st.expander("🔧 Solutions to Improve Cost Fairness"):
                ...
                **Your allocation has high cost inequality.** Here are evidence-based solutions:
                ...
                - **Action**: Go to ⚙️ Configuration in the sidebar, set Min Preference to "None" and try again
                ...
                - **Action**: Go to ⚙️ Configuration in the sidebar, change "Unranked Topic Cost" to lower value
                ...
                - **Solution**: Go to ⚙️ Configuration in the sidebar, increase P_topic and P_coach penalties
            ...
            with st.expander("🔧 Suggestions to Improve Cost Fairness"):
                ...
                **Quick Wins:**
                - Reduce unranked cost (⚙️ Configuration page: Unranked Topic Cost: try 50-75)
